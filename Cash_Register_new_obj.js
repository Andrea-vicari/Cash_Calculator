alert(`
!!) Occhio al caso paid not filled e numero negativo, lettere ok!!
1) CLONARE CR.JS TOGLIENDO CONSOLE.LOG, LASCIANDONE UNA CON UNA SENZA
2) STILARE I MODALI (Finire)
3) VEDERE SE FARE IL cid MODIFICABILE TRAMITE MANAGER
4) VEDERE SE ESPORTARE IL REPORT GIORNALIERO
5) INCLUDERE ALLA FINE DEL VIDEO ISTRUZIONI TUTTI CASI DI ALERT
`)
...
// Set a global counter to be ++ each time the user close the POPUP
// generated by clicking CALCULATE button
//
var counter = 0;

// Declare initial CID but wait to assign the value, depending on the URL
var initialCID;
...
// Set to current URL to window location
// Ex. currentURL ==> http://127.0.0.1:5500/register.html?1
// or ==> http://127.0.0.1:5500/register.html

let currentURL = window.location.href;

// Split current URL into an array
let arrayedURL = currentURL.split('');
// Get the last letter of the array
let lastLetter = arrayedURL[arrayedURL.length-1];

// If the last letter is l, it means that now one clicked on the closure
// button of the pop up, otherwise an ?number will be present
if(lastLetter == "l"){

// Initial CID is the initial situation of the Drawer BEFORE the transaction
initialCID = [['ONE HUNDRED', 0],['TWENTY', 0],['TEN', 0], ['FIVE', 0], ['ONE', 1], ['QUARTER', 0], ['DIME', 0],['NICKEL', 0],['PENNY', 0]];

var startCID = [...initialCID];
let stringedInitialCID = JSON.stringify(startCID);
...
// Important, if the last letter is not l, assign to initialCID the CID
// Imported via json (generated by a transaction)
else{
    initialCID = [['ONE HUNDRED', 0],['TWENTY', 0],['TEN', 0], ['FIVE', 0], ['ONE', 1], ['QUARTER', 0], ['DIME', 0],['NICKEL', 0],['PENNY', 0]];
    var startCID = [...initialCID];
    let stringedInitialCID = JSON.stringify(startCID);
...
function getThePrice(){
    priceAr = Number(document.getElementById("TotalBill").value).toFixed(2);
    // PRIMA PARTE X IL CHECK
    finalObjToCheck = new FinalConstructor(priceAr);
    console.log(finalObjToCheck)
}
...
function refreshPageAndADD(){
    counter++;
    window.location = `/index.html?${counter}`;
}
...
function FinalConstructor(price,cash){
    this.price = price;
    this.cash = cash;
}
...
document.getElementById("video_modal").classList.remove("d-block");
}
...
document.getElementById("video_modal").classList.add("d-block");
}
